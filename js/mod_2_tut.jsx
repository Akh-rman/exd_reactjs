...
class Game extends React.Component {
    ...
    render() {
        ...
        return (
            <div style = {style}>
                <h1>{this.state.winner > 0 ? this.state.winner = 1 ? "Blacks Turn" : "Red Turn" : this.state.player ? "Blacks Turn" : "Red Turn"}</h1>
                <Board cells = {this.state.cells} handleClick = {this.handleClick}/>
                <button style = {styleButton} onClick = {() => this.restart()}>Restart</button>    
            </div>
        )
    }
    ...
    checkDiagonal(row, col) {
        ...
        while (rR >= 3 && cR >= 3) {
            if (c[rR][cR] == val && c[rR - 1][cR - 1] == val && c[rR - 2][cR - 2] && c[rR - 3][cR - 3] == val) {
                return 1;  
            }
            rR--;
            cR--;
